...
if ($action == 'read') {
    ...
    if(floatval(DOL_VERSION) <= 8.0) {
        $searchSoc = $socStatic->searchByName($datas['buyer_company'], 0, $filterSearch); // Retourne un tableau ou -1 en cas d'erreur
    }
    else {
        $searchSoc = $socStatic->fetch('', $datas['buyer_company']);  // Retourne -2 si on trouve plusieurs Tiers
    }
	if($searchSoc < 0) {
